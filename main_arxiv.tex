...
The learnable per-token quantities should be the memory transition and the information
decay. Let $\ve{x}_t$ denote the current token representation at this layer. A simple
parameterization is to produce both gates from $\ve{x}_t$ by learned projections and map
them into the valid ranges
\begin{equation}
  \ve{\alpha}_t=\sigma(\ve{W}_{\alpha}\ve{x}_t+\ve{b}_{\alpha}),
  \qquad
  \ve{\lambda}_t=\sigma(\ve{W}_{\lambda}\ve{x}_t+\ve{b}_{\lambda}),
  \label{eq:kla-parameterization}
\end{equation}
possibly with small lower bounds for numerical stability. The vector $\ve{\alpha}_t$
parameterizes the deterministic memory transition $\ve{D}_t=\diag(\ve{\alpha}_t)$, while
$\ve{\lambda}_t$ parameterizes process drift through the information update
\eqref{eq:diag-info-scan}. In this sense, $\ve{\lambda}_t$ takes over the learnable role
of ``how strongly should this token edit memory?'' from the scalar write gate $\beta_t$
used in GDN and KDA, but indirectly: it changes uncertainty, and then the gain
$\ve{\kappa}_t$ becomes the actual adaptive write direction and strength. The observation
noise $r$ can be a fixed positive scalar or a learned positive scalar per head.
...
diagonal memory transition and an information decay,
\begin{equation}
\begin{aligned}
  \ve{\alpha}_t &= \sigma(\ve{W}_{\alpha}\ve{x}_t+\ve{b}_{\alpha}),
  &
...
\end{equation}
It then computes the adaptive gain from prefix information statistics, rather than from a
full Riccati recursion:
\begin{equation}
\begin{aligned}
  \widehat{\ve{c}}_t
  &= \ve{\lambda}_t\odot\ve{c}_{t-1},
  &
  \widehat{\ve{p}}_t
  &= \ve{\alpha}_t^2\odot\frac{1}{\epsilon+\widehat{\ve{c}}_t}+\ve{q}_t,
  \\[3pt]
  ...
  &
  \ve{c}_t
  &= \widehat{\ve{c}}_t+\frac{1}{r}\,\ve{k}_t\odot\ve{k}_t .
\end{aligned}
