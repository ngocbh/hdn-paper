...
\eqref{eq:kla-box-memory} becomes
...
and for diagonal/isotropic comparison checks; it is not the trained fast path.
...
\end{equation}
initialized with $\ve{S}_0=\ve{0}$ and isotropic prior covariance $\ve{P}_0=\ve{I}$. It
...
$\ve{P}_t\leftarrow\tfrac12(\ve{P}_t+\ve{P}_t^{\tr})$. This dense filter is a reference for the
posterior mean, gain, and covariance, but it is not equivalent to the diagonal variational state.
In general, the variational variance
$p_{t,i}^{\mathrm{VI}}=1/(\ve{P}_t^{\star-1})_{ii}$ differs from the exact marginal variance
$P_{t,ii}^{\star}$. Likewise, the protection scale $\mu$ in \eqref{eq:diag-info-update} applies
only to information carried into future tokens; replacing $r_t$ by $r_t/\mu$ in this dense filter
would also change the current gain and would define a different observation model.
...
the scalar uncertainty through the transition, apply a calibrated scalar protection update, and read
...
calibrated per-channel protection increment is
\begin{equation}
  c_{t,i}
  =
  \widehat{c}_{t,i}+d_k\frac{\ve{k}_{t,i}^2}{r_t},
  \qquad i=1,\dots,d_k .
...
\end{equation}
which is the scalar protection update. It is the isotropic average of the same calibrated
coordinate increments used by the diagonal tracker. Equivalently, the isotropic summary of the
calibrated rank-one information surrogate
$(d_k/r_t)\ve{k}_t\ve{k}_t^{\tr}$ is its trace average:
\begin{equation}
  u_t
  =
...
no information to the measurement. The unscaled rank-one precision term belongs to the exact
dense Kalman update; after calibration and restriction to an isotropic scalar state, the trace
average is the invariant scalar summary used for future protection.
...
be computed by an associative scan over $2\times2$ matrices, with $c_0=1$.
...
\S\ref{sec:kla} from the diagonal covariance recursion. The dense one-step posterior is
approximated by the mean-field variational solution in
\eqref{eq:diag-variational-objective}--\eqref{eq:diag-variational-solution}. With an independently
...
inverses, and ratios are coordinatewise in the diagonal state; we then write a single
...
$\ve{c}_t$ requires an approximation. The one-step ELBO gives the variational update
$\ve{c}_t^{\mathrm{VI}}=\widehat{\ve{c}}_t+\ve{k}_t\odot\ve{k}_t/r_t$. Diagonal KDN then
decouples the information passed to later tokens from the current write using the information
scale $\mu$:
\begin{equation}
  \ve{c}_t=\widehat{\ve{c}}_t+\frac{\mu}{r_t}\,\ve{k}_t\odot\ve{k}_t,
  \qquad
  \ve{c}_0=\ve{1}.
...
\begin{equation}
  c_t=\widehat{c}_t+\mu\frac{k_t^2}{r_t},
  \qquad
  \widehat{c}_t:=\frac{1}{\widehat{p}_t}.
  \label{eq:app-update}
\end{equation}
The variational filter is $\mu=1$. Diagonal KDN uses $\mu=d_k$ by default; for normalized keys
with roughly uniform coordinate energy, this gives
...
\begin{equation}
  ...
  \label{eq:app-chat}
\end{equation}
so the decoupled information-state recurrence is
\begin{equation}
  c_t=\frac{c_{t-1}}{\alpha_t^2+\omega_t\,c_{t-1}}+\mu\frac{k_t^2}{r_t}.
  \label{eq:app-cfull}
\end{equation}
...
\begin{equation}
  ...
  =\begin{bmatrix}1+\mu\tfrac{k_t^2}{r_t}\,\omega_t & \mu\tfrac{k_t^2}{r_t}\,\alpha_t^2\\[3pt] \omega_t & \alpha_t^2\end{bmatrix}.
  \label{eq:app-mobius}
\end{equation}
...
\begin{equation}
  ...
  \qquad
  c_t=\lambda_t\,c_{t-1}+\mu\frac{k_t^2}{r_t},
  \label{eq:app-affine}
\end{equation}
...
exactly cancelled by this $\omega_t$. We initialize $c_0=1$; $\mu$ controls only the information
increment contributed by each token.
