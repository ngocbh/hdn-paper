...
tokens, used for a controlled mixer ablation, and a $750$M-parameter model trained on $50$B
tokens. At each scale all mixers share the same backbone, optimizer, and data order, and are
...
the best perplexity and the best average. The ordering sharpens at $750$M/$50$B: the Isotropic KDN
is best on WikiText, LAMBADA, and every commonsense task except LAMBADA accuracy (where it ties
KDA), lifting average accuracy to $54.5$ versus $53.9$ for KDA and $51.5$ for GDN-2.
...
KDA             & \underline{18.85} & \underline{15.06} & \textbf{44.23} & \underline{71.00} & \underline{51.23} & \underline{54.93} & \underline{67.63} & \underline{34.13} & \underline{53.86} \\
Isotropic KDN   & \textbf{18.42} & \textbf{14.67} & \underline{44.21} & \textbf{71.16} & \textbf{52.14} & \textbf{55.96} & \textbf{68.18} & \textbf{35.15} & \textbf{54.47} \\
\bottomrule
\end{tabular}
...
\note{Diagonal KDN at 750M is still training; row to be added.}}
\label{tab:pretrain-lm}
\end{table}

\paragraph{In-context retrieval.}
Table~\ref{tab:pretrain-niah} reports single- and multi-key needle-in-a-haystack accuracy from
RULER~\citep{hsieh2024ruler}. At $220$M/$20$B retrieval is near chance for every mixer: the models
are too small and undertrained to store high-entropy needles reliably, so these numbers are noisy
and we include them only for completeness. The separation emerges at $750$M/$50$B, where both KDA
and the Isotropic KDN dramatically outperform GDN-2 (retrieval average $52$--$53$ versus $30$). On
the interference-heavy S-NIAH-3 the Isotropic KDN reaches $62.4$ against $3.6$ for GDN-2 and $48.4$
for KDA, while KDA leads the simpler S-NIAH-2 and multi-key settings---consistent with the Kalman
gain protecting existing associations under interference.
...
\toprule
Model & S-NIAH-2 @4K & S-NIAH-3 @2K & MK-NIAH-1 @4K & Avg. \\
\midrule
\multicolumn{5}{@{}l}{\emph{220M parameters, 20B tokens}}\\
DeltaNet        & 7.6  & \textbf{4.4} & 17.8 & 9.9 \\
Gated DeltaNet  & 14.8 & 0.8 & \textbf{22.4} & \underline{12.7} \\
GDN-2           & \underline{19.4} & 0.2 & 16.0 & 11.9 \\
KDA             & 12.8 & \underline{3.2} & 11.6 & 9.2 \\
Diagonal KDN    & \textbf{32.0} & 0.2 & \underline{18.0} & \textbf{16.7} \\
Isotropic KDN   & 11.0 & 2.2 & 14.8 & 9.3 \\
\midrule
\multicolumn{5}{@{}l}{\emph{750M parameters, 50B tokens}}\\
GDN-2           & 61.2 & 3.6 & 25.0 & 29.9 \\
KDA             & \textbf{82.0} & \underline{48.4} & \textbf{27.6} & \textbf{52.7} \\
Isotropic KDN   & \underline{66.8} & \textbf{62.4} & \underline{26.4} & \underline{51.9} \\
\bottomrule
\end{tabular}
\caption{In-context retrieval accuracy (\%) on RULER single- and multi-key needle-in-a-haystack
tasks~\citep{hsieh2024ruler}. At $220$M/$20$B all mixers are near chance (undertrained); meaningful
separation appears at $750$M/$50$B. Best per column within each block in \textbf{bold}, second best \underline{underlined}.
\note{Diagonal KDN at 750M pending.}}
...
\begin{proof}[Proof of Proposition~\ref{prop:kalman-optimal-update}]
Fix a token $t$ and write
\[
  \widehat{\ve{S}}=\widehat{\ve{S}}_t,\qquad
  \widehat{\ve{P}}=\widehat{\ve{P}}_t,\qquad
  \ve{k}=\ve{k}_t,\qquad
  \ve{v}=\ve{v}_t,\qquad
  r=r_t .
\]
The prediction step is the standard linear--Gaussian time update. If the prior estimation
error has covariance $\ve{P}_{t-1}$, then after the transition
$\ve{S}_t^\star=\ve{D}_t\ve{S}_{t-1}^\star+\ve{W}_t$ the predicted mean and covariance are
\[
  \widehat{\ve{S}}_t=\ve{D}_t\ve{S}_{t-1},
  \qquad
  \widehat{\ve{P}}_t
...
\]
because $\ve{W}_t$ is zero mean with covariance $\ve{\Omega}_t$ and is independent of the
previous estimation error.

It remains to choose the residual-write direction. Consider an arbitrary candidate
$\ve{\kappa}\in\R^{d_k}$ and the update
\[
  \ve{S}^+
  =
  \widehat{\ve{S}}
  +
  \ve{\kappa}\big(\ve{v}-\widehat{\ve{S}}^{\tr}\ve{k}\big)^{\tr}.
\]
For any value coordinate, let $\ve{\epsilon}$ denote the predicted key-space estimation error
and let $\eta$ denote the scalar observation noise in that coordinate. Then
$\E[\ve{\epsilon}]=0$, $\Cov(\ve{\epsilon})=\widehat{\ve{P}}$, $\E[\eta]=0$,
$\Var(\eta)=r$, and $\eta$ is independent of $\ve{\epsilon}$. The updated estimation error is
\[
  \ve{\epsilon}^+
  =
  (\ve{I}-\ve{\kappa}\ve{k}^{\tr})\ve{\epsilon}
  -
  \ve{\kappa}\eta .
\]
Thus the posterior covariance induced by this candidate write direction is
\[
  \ve{P}^+(\ve{\kappa})
  =
  (\ve{I}-\ve{\kappa}\ve{k}^{\tr})\widehat{\ve{P}}
  (\ve{I}-\ve{\kappa}\ve{k}^{\tr})^{\tr}
  +
  r\,\ve{\kappa}\ve{\kappa}^{\tr},
\]
which is exactly the uncertainty objective in \eqref{eq:kalman-tracking-objective}.

Now minimize its trace. Since $\widehat{\ve{P}}$ is symmetric positive semidefinite,
\[
\begin{aligned}
  \Trace\!\left(\ve{P}^+(\ve{\kappa})\right)
  &=
  \Trace(\widehat{\ve{P}})
  -2\ve{\kappa}^{\tr}\widehat{\ve{P}}\ve{k}
  +
  \big(r+\ve{k}^{\tr}\widehat{\ve{P}}\ve{k}\big)\ve{\kappa}^{\tr}\ve{\kappa}.
\end{aligned}
\]
This is a strictly convex quadratic in $\ve{\kappa}$ because
$r+\ve{k}^{\tr}\widehat{\ve{P}}\ve{k}>0$. Setting its gradient to zero gives
\[
  -2\widehat{\ve{P}}\ve{k}
  +
  2\big(r+\ve{k}^{\tr}\widehat{\ve{P}}\ve{k}\big)\ve{\kappa}
  =
  \ve{0},
\]
and therefore the unique minimizer is
\[
  \ve{\kappa}^\star
  =
  \frac{\widehat{\ve{P}}\ve{k}}
       {r+\ve{k}^{\tr}\widehat{\ve{P}}\ve{k}} .
\]
Restoring the time index gives the gain in \eqref{eq:kalman-gain}.

Substituting this minimizing gain into the residual write gives
\[
  \ve{S}_t
  =
  \widehat{\ve{S}}_t
  ...
  \big(\ve{v}_t-\widehat{\ve{S}}_t^{\tr}\ve{k}_t\big)^{\tr}.
\]
For the covariance, let
$s=r+\ve{k}^{\tr}\widehat{\ve{P}}\ve{k}$ and
$\ve{u}=\widehat{\ve{P}}\ve{k}=s\ve{\kappa}^\star$. The Joseph-form covariance becomes
\[
\begin{aligned}
  \ve{P}^+(\ve{\kappa}^\star)
  &=
  \widehat{\ve{P}}
  -\ve{\kappa}^\star\ve{u}^{\tr}
  -\ve{u}\ve{\kappa}^{\star\tr}
  +s\,\ve{\kappa}^\star\ve{\kappa}^{\star\tr}  \\
  &=
  \widehat{\ve{P}}
  -\ve{\kappa}^\star\ve{u}^{\tr}
  =
  (\ve{I}-\ve{\kappa}^\star\ve{k}^{\tr})\widehat{\ve{P}} .
\end{aligned}
\]
This is the covariance update stated in the proposition. The read
$\ve{o}_t=\ve{S}_t^{\tr}\ve{q}_t$ is then the linear-attention read from the posterior memory
estimate.
\end{proof}
