...
The trained-checkpoint gain direction is remarkably stable: nominal gain anisotropy stays
...
but strongly reduces the mean channelwise coefficient $\overline\beta_{\rm ch}$.  Trained
DiagKLA therefore exhibits a robustly anisotropic gain direction, while
\code{info\_scale} primarily controls how strongly
...
effective rank $0.72$--$0.52$.  It is accompanied by only a $2.8\%$--$4.3\%$ relative
lower tail in $\beta_{\rm ch}$ under the cold-channel threshold; that diagnostic does not
exclude a few hot channels or uniform absolute shrinkage.
...
anisotropy, and decreases effective rank.  It also reduces the mean channelwise gain
coefficient, but changes gain direction much less.  The crossed grid makes these runtime
effects visible at fixed weights.  Cross-checkpoint trends are additionally consistent with
learned compensation, but those
...
intervention.  Consequently this report shows how scale interventions covary with anisotropy
and filtering/write geometry, but does not claim that anisotropy itself causes an accuracy
improvement.
...
state-derived predicted covariance.  Each nominal run seals its summary, NPZ arrays, six
plots, exact source closure, environment, checkpoint identity, and token identity.  Each
crossed cell seals its summary, trained NPZ, completion record, and the same provenance
classes without per-cell plots.  All artifact identities use SHA-256 hashes.
...
information $c$.  Mean channelwise gain coefficient and effective write strength also fall.
The joint pattern is consistent with partial learned compensation, but does not cancel the
covariance contraction associated with scale.
...
\subsection{Covariance geometry in trained checkpoints}
...
Relative to the canonical controls, trained checkpoints exhibit far less scale dependence
in gain direction: trained $\Akap$ spans only $0.637$--$0.650$, while the controls fall
monotonically from $0.527$ to $0.319$.  The trained-minus-control posterior-anisotropy
differences are $0.089$, $0.190$, $0.189$, and $0.138$ for increasing scale.  This is
consistent with learned channel selectivity,
although the controls are not exact replays of historical initialization.

\subsection{Depth dependence}

The depth profiles distinguish gain direction from covariance depth structure.  From the first
...
at $s=1$.  Gain anisotropy remains much flatter.  Deeper layers therefore exhibit stronger
high-scale covariance concentration.  Each layer owns a separate recurrent covariance state,
so this profile reflects layer-specific dynamics connected by hidden activations, not one
covariance state accumulating across layers.
...
anisotropy rises only $0.025$--$0.036$, while the mean channelwise gain coefficient falls by
...
\begin{enumerate}
  \item \textbf{Trained checkpoints exhibit directional selectivity.}  Gain anisotropy lies
  near $0.64$ across the four nominal scales, far above the high-scale canonical controls.
  The gain is therefore not well summarized as a scalar multiple of the key.
  ...
  The posterior becomes smaller, more unequal across channels, and lower rank.  The mean
  channelwise gain coefficient falls strongly, while the gain's off-key fraction changes only
  slightly.
  \item \textbf{High-scale concentration is stronger in deeper layers.}  The deeper layers
  of the $s=128$ and $s=512$ checkpoints show greater posterior anisotropy, whereas low-scale
  profiles are nearly flat.  Covariance state persists across tokens within each layer;
  hidden activations, rather than a shared covariance state, connect different layers.
\end{enumerate}

The low cold-channel fraction is a narrow auxiliary diagnostic.  Only $2.6\%$--$5.3\%$ of
crossed cells' $\beta_{\rm ch}$ values fall below one tenth of their within-head median.
Thus high anisotropy and lower effective rank are not accompanied by a large relative lower
tail in the gain coefficient under this threshold.  The metric cannot distinguish distributed
covariance heterogeneity from a few hot channels, nor can it detect uniform absolute shrinkage.
...
Trained DiagKLA checkpoints exhibit strongly channel-dependent covariance and gain.  The
stable gain anisotropy near $0.64$ shows that this behavior persists across a 512-fold nominal
scale range in these checkpoints.  Information scale nevertheless remains a powerful state-control
