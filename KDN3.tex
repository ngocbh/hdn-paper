...
KDN-3 to Gated DeltaNet-2: both expose key- and value-axis modulation, but KDN-3 ties erasure and
...
Expanding the KDN-3 mean update in Equation~\eqref{eq:kdn3-mean-update} gives
\begin{equation}
  \ve{S}_t^{\mathrm{KDN3}}
  =\widehat{\ve{S}}_t
  ...
  \qquad
  \ve{g}_t=\widehat{\ve{P}}_t\ve{k}_t.
  \label{eq:kdn3-expanded-gdn2-comparison}
\end{equation}
...
\begin{tabular}{@{}llll@{}}
  ...
  KDN-3 & $\ve{g}_t=\widehat{\ve{P}}_t\ve{k}_t$ & $\ve{k}_t$ & $\ve{\beta}_t$ \\
  \bottomrule
\end{tabular}
\end{center}
For KDN-3, the same $\beta_{t,j}$ multiplies the observation write and the erasure of the predicted
...
The two covariance factors explain the two visible parts of the KDN-3 update.  The key factor
...
\eqref{eq:kdn3-expanded-gdn2-comparison} does not imply that either family contains the other.
Their common intersection is much smaller.

\begin{proposition}[Algebraic GDN-2--KDN-3 intersection]
At the level of the memory-update equations, assume a nondegenerate update with
$\ve{k}_t\ne\ve{0}$ and require
Equations~\eqref{eq:gdn2-update} and \eqref{eq:kdn3-expanded-gdn2-comparison} to agree for arbitrary
...
\end{aligned}
  \label{eq:gdn2-kdn3-intersection}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:gdn2-kdn3-write-condition}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:gdn2-kdn3-erase-condition}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:gdn2-kdn3-scalar-intersection}
\end{equation}
which is the KDA form.  Away from this intersection, KDN-3 can rotate the write direction through
key-space uncertainty, which GDN-2 cannot express with its fixed outer direction $\ve{k}_t$;
GDN-2 can independently choose erase and write gates, which a coherent KDN-3 innovation cannot
express.

The one-axis boundaries expose the two separate mismatches.  When
$\widehat{\ve{P}}_t\propto\ve{I}_K$ but $\ve{\beta}_t$ remains value dependent, KDN-3 reduces to
...
$\widehat{\ve{B}}_t\propto\ve{I}_V$ and $\ve{R}_t\propto\ve{I}_V$, KDN-3 reduces to Diagonal KDN:
...
KDN-3 instead derives its update from the separable predictive covariance and diagonal observation
