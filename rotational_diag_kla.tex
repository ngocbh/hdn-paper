...
no remaining issue and approved the strict full S7 repetition.  No S6 closure is reused.  The exact
...
manifest, worktree, and, for the anchor, the unchanged data manifest.
...
sequence is:
\begin{enumerate}[leftmargin=1.8em]
  \item complete the in-flight S7 numerical and actual-anchor preflights, retain and review their
        new closures, then run one S7-bound six-arm allocation and its fresh-node validator; the
        S6 in-job result remains non-authorizing corroboration only;
  ...
  \item create the separate required production snapshot and invoke the
        \pathcode{prepare-fresh} mode of \pathcode{submit_rdiag_shared_dt.py} only for the new
        versioned \pathcode{rdiag_kla_shared_dt_v2_750M} with train configuration
...
this point, the optimistic path requires one complete S7 source-bound candidate cycle followed by
...
gate workloads, and six external attestors), after the already-submitted S7 preflight pair the
comparable operational lower bound is at least 22 further scheduler submissions before production
submission: S7 main, S7 validator, and the 20-job gate control plane.  A held-out replication would
increase that count.
