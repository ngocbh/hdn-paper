...
Model, Mamba-3 Lessons, and an Implementation Contract}
\author{}
\date{August 28, 2026}
...
needed for promotion.  A step-520 screen after 519 completed optimizer updates accepted the E2
clock design, but the optimized composed
implementation still requires its complete post-integration correctness, distributed, soak, and
performance qualification; this document does not declare production readiness.
\end{abstract}
...
\end{align*}
The direct radial backward has no angle input.  Conversely, the clock's $m$ and $a$ arguments are
detached, so the phase VJP returns no gradient to the radial gate, radial bias, or $A^{\log}$.
...
\begin{enumerate}[leftmargin=1.8em]
  ...
  \item in the RDiag direct radial kernel, recompute the rotary prefix using
        Eq.~\eqref{eq:pair-radial}, preserve the tail, and build the log-decay prefix consumed by
        the chunked memory kernel;
  \item form the exact detached FP32 radial mean $m$ described below;
  \item keep $Wx$ as the existing FP32 GEMM, then fuse the pointwise clock chain and
        Eq.~\eqref{eq:rdiag-delta};
...
then apply the tanh derivative.  The validity mask is applied to the incoming clock cotangent
before the sigmoid/tanh chain, matching the eager \texttt{where(valid, raw\_tau, detached\_nan)}
VJP.  After the FP32 head reduction and factor order in Eq.~\eqref{eq:dangle-order}, the raw-angle
gradient is stored in \texttt{angle\_raw}'s dtype (BF16 on the production route) before the angle
projection VJP.  Weight and hidden-state gradients then flow through the ordinary linear
operations.
...
\end{enumerate}
FSDP mixed precision must not downcast this buffer.  Resume tests must compare its bytes before and
after round trips and verify that fresh construction cannot silently overwrite a checkpointed
anchor.
...
\begin{enumerate}[leftmargin=1.8em]
  ...
  \end{align*}
  Pair-average $d^{\rm radial}$ and $\omega$ on the rotary prefix, and construct $\alpha$ from
  Eq.~\eqref{eq:pair-radial}.  Do not use \texttt{clock\_dt} for radial dynamics.

  \item \textbf{Bounded clock and phase.}
  \begin{align*}
    z^{\rm clk}&=\texttt{linear}(x,W)/\sqrt D+\beta,\\
    \tau&=0.1\,\sg\bigl(\log m-\log(0.1-a)+\tanh z^{\rm clk}\bigr),\\
    \delta&=\pi\tanh(e)\,\tau,\\
  ...
  return no clock gradient to $m$ or $a$; and run the independent radial/Kalman VJPs.
\end{enumerate}
...
isolation numbers for the v1 geometry, not a speed claim for the final E2 composition; the exact
clock mean's FP32 scratch and full-layer runtime still require measurement.
...
\section{Invariants and promotion checklist}
...
\begin{enumerate}[leftmargin=1.8em]
  ...
  \item the displayed forward and backward reduction orders are preserved; and
  \item the unrotated tail is an exact pass-through of diagonal KLA.
\end{enumerate}

Before production promotion, the following checklist should be closed:
\begin{enumerate}[leftmargin=1.8em]
  ...
  \item FSDP construction, buffer dtype, state-dict, save/resume, and rank-consistency tests;
  ...
  \item long-sequence and long-run soak tests beyond the step-520 design screen;
  \item peak-memory and end-to-end throughput measurements against the same recipe, including the
        cost of the materialized FP32 clock tensor; and
  \item the full downstream evaluation suite after a naturally completed training run.
\end{enumerate}
...
The stable implementation statements in this note were audited against Git commit
\texttt{2ff0a4987a1b40f627fbd3efd1fb16ff0490e307} (tree
\texttt{2fd5560d8337523383c20dda503eb0098f996dbc}).  Relevant immutable commit/path regions are:
\begin{itemize}[leftmargin=1.8em]
  ...
  \item \pathcode{lit_gpt/kla_ops/rdiag_clock.py}, anchor and eager oracle around 72--119 and
        190--270, fused clock around 283--388, and custom backward around 621--722;
  \item \pathcode{lit_gpt/kla_ops/rdiag_direct.py}, precise clock softplus around 43--47, direct
        radial transform around 300--377, and isolated radial VJP beginning around 702;
...
\end{itemize}
At that commit the source SHA-256 values, in the same order as the list above, are:
\begin{quote}
\ttfamily\footnotesize
8f09aa5881681adc9fa958f109f0ebf07240bad7b05877376f7c1f76633b9879\\
b15293c665f941002525ce6ccbf346265c13a83bb4ca1e2f4a0a276289c8bd0c\\
95b4309cd07d3e55507b4be623769383451cbf0e72f1624a038c985dba3b1ac4\\
647dda58e7855f3139c460afab7db97242cc18dfd5c5dc29e44b4497bda68f30\\
8918f21b3d326e3387001595b709a54b30b0ad944a8760b4d83d4cbd07e3b49a\\
261489c0076bc4752fa909319917fd14037471dea5416300d34cb1319d35c1b8\\
0f9ad9598188979f3eab6e3b3872de109409a358751198f664d4ad5717b6393e\\
431be42bf1d8903a675d8ca4a6ff06afdda86516f77d2258f0aa556747ccee86\\
fbacfa199a38ac4fe58c18612e3a0971bb0acdcd6cb7dd126b4bc85caabfb8b3\\
84634c19916fa88bf0e4b7849d4e71e8984fe3e7f385bfc0ada726a5b7eaf02c\\
e352c78a009b0b24f0290429c16c5213e004b61aeec4fc0a3eb2a7636918a910\\
c8004393867cdc7c638117a444db74030e7abfabe55c79f795468d27e8561d4c\\
7f9a75e0237cb1c225da7734a2cbc903257687e368e4b675f8826d5889c072dc
\end{quote}

The exact-mean follow-up is commit
\texttt{fe1b1258f601092ab29ae3431ec5a703d7087b25} (tree
\texttt{bade1969461e68b8a987343ead8cc0f8634ac9e4}).  In that commit,
\pathcode{lit_gpt/kla_ops/rdiag_direct.py} has SHA-256
\begin{quote}
\ttfamily\footnotesize
7252664763c8d3b3b7af424b194f210424896f4d43f92ba39ef52a38a11f5628
\end{quote}
The precise softplus is around lines 43--47, FP32 clock materialization around 299--375, and the
framework mean around 1155--1197.  This is the frozen source for the
materialize-plus-\texttt{torch.mean} contract in Eq.~\eqref{eq:torch-mean}.
H200 job 1777082 subsequently completed with 17 passed and 3 failed tests.  The exact radial-clock
bitwise tests passed for $K=64$ and $K=128$, including threshold/extreme inputs; fused operator
parity, backward-association checks, finite full-layer checks, and every legacy performance gate
also passed.  Each of the three sealed-reference full-layer comparisons stopped at its first
mismatch, \texttt{layer.rdiag\_angle\_proj.weight}, for the $(H,D,K)=(16,1024,64)$,
$(16,2048,128)$, and $(18,2304,128)$ geometries; later VJPs were not reached.  Thus exact-mean
operator qualification is positive, but full-layer sealed-reference parity---and therefore
...
the specific observations in Section~\ref{sec:lessons}.  The completed exact-mean H200 result is in
\pathcode{/checkpoints/ngocbh/hdn/ckpts/logs/rdiag_clock_exact_1777082.log}.  These results do not
close the production checklist above.
